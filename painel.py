...
# -------------------- CONFIGURAÇÕES ----------------------
titulo_pagina = 'Painel de Eventos Climáticos :world_map:'
layout = 'wide'
st.set_page_config(page_title='Painel de Eventos Climáticos', layout=layout, initial_sidebar_state="collapsed")
...
with tabs[1]:
    ...
    # METRICAS1
    lr = psrQ3.groupby(['uf'], as_index=False)[['valor_premio', 'valor_indenizacao']].sum()
    lr['loss_ratio'] = lr.valor_indenizacao / lr.valor_premio

    col_config3.metric('Total de Apólices', len(psrQ3))
    ...
    # MAPA SINISTRALIDADE
    sin_muni = psrQ3.groupby(['ibge'], as_index=False)[['valor_premio', 'valor_indenizacao']].sum().copy()
    sin_muni['loss_ratio'] = (sin_muni.valor_indenizacao / sin_muni.valor_premio) * 100
    ...
    bar_data = psrQ3.groupby(['ano', psrQ1.data_apolice.dt.month]).num_apolice.size()
    ...
    line_data = psrQ3.groupby(['ano', psrQ1.data_apolice.dt.month])[['valor_premio', 'valor_indenizacao']].sum().copy()
    ...
    line_data['loss_ratio'] = (line_data.valor_indenizacao / line_data.valor_premio) * 100
    fig_bar.add_trace(
